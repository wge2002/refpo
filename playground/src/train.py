import datetime
import time
from typing import Annotated, Literal, Union # 1. 引入 Literal 和 Union
...
from flow_policy import fpo,rfpo,rfpo02, rollouts


def main(
    # TODO: these envs are just what i'm testing with initially
    method: Literal["fpo", "rfpo", "rfpo02"],  # 2. 添加 method 参数
    env_name: Annotated[
        str,
        tyro.conf.arg(
    ...
    wandb_entity: str,
    wandb_project: str,
    config: Union[fpo.FpoConfig, rfpo.RfpoConfig, rfpo02.Rfpo02Config],
    ...
    # 4. 核心逻辑分发：根据 method 选择对应的 State 初始化方法
    # 假设 rfpo 和 rfpo02 也有类似的 .init 接口
    prng_key = jax.random.key(seed)
    
    if method == "fpo":
        agent_state = fpo.FpoState.init(prng=prng_key, env=env, config=config)
    elif method == "rfpo":
        # 假设 rfpo 模块中有 RfpoState
        agent_state = rfpo.RfpoState.init(prng=prng_key, env=env, config=config)
    elif method == "rfpo02":
        # 假设 rfpo02 模块中有 Rfpo02State
        agent_state = rfpo02.Rfpo02State.init(prng=prng_key, env=env, config=config)
    else:
